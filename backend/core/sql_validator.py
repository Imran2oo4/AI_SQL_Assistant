...
class SQLValidator:
    ...
    def _validate_schema_references(self, statement: Statement) -> Tuple[bool, Optional[str]]:
        ...
        """
        try:
            # Extract table names from FROM and JOIN clauses ONLY
            tables = self._extract_tables(statement)
            
            # Verify all tables exist in the database schema
            for table in tables:
                if table not in self.table_names:
                    return False, f"Table '{table}' does not exist in the database schema. Available tables: {', '.join(sorted(self.table_names))}"
            
            # If no tables found, something is wrong with the query
            if not tables:
                return False, "No valid table found in FROM clause"
            
            # Extract column references (simplified validation)
            ...
            for col, table_hint in columns:
                # Skip special keywords, SQL functions, and aggregates that might be parsed as columns
                skip_terms = ['select', 'from', 'where', 'order', 'group', 'having', 'limit', 'asc', 'desc', 'by',
                              'max', 'min', 'count', 'sum', 'avg', 'distinct', 'as', 'and', 'or', 'not', 'in', 'like']
                if col in skip_terms:
                    continue
                    
                if table_hint:
                    # Qualified column reference (table.column)
                    # Make sure the table_hint is actually a valid table, not a keyword
                    if table_hint in self.table_names:
                    ...
                    # If table_hint is not a valid table, treat the whole thing as an unqualified column
                    elif not self._column_exists_in_any_table(col, tables):
    ...
    def _extract_tables(self, statement: Statement) -> List[str]:
        """Extract table names from SQL statement - ONLY from FROM and JOIN clauses."""
        tables = []
        from_seen = False
        join_seen = False
        
        for token in statement.tokens:
            # First check if we should stop looking for tables
            if token.ttype is Keyword and token.value.upper() in ('WHERE', 'ORDER', 'GROUP', 'HAVING', 'LIMIT', 'BY'):
                from_seen = False
                join_seen = False
                continue
            
            # Check if we should start looking for tables
            ...
            # Extract table names when we're in FROM or JOIN context
            if from_seen or join_seen:
                if isinstance(token, Identifier):
                    table_name = token.get_real_name()
                    if table_name:
                        # Clean up table name - remove aliases and whitespace
                        table_name = table_name.split()[0] if ' ' in table_name else table_name
                        tables.append(table_name.lower())
                        from_seen = False  # Got the table, stop looking
                        join_seen = False
                    ...
                    for identifier in token.get_identifiers():
                        ...
                        if table_name:
                            table_name = table_name.split()[0] if ' ' in table_name else table_name
                            tables.append(table_name.lower())
                    from_seen = False  # Got all tables, stop looking
                    join_seen = False
    ...
    def _extract_columns_recursive(self, tokens, columns, skip_next_identifier=False):
        ...
        while i < len(tokens):
            ...
            # Reset skip flag after WHERE, ORDER BY, etc - these indicate column references
            if token.ttype is Keyword and token.value.upper() in ('WHERE', 'ORDER', 'GROUP', 'HAVING', 'LIMIT', 'BY'):
    ...
    def _parse_identifier(self, identifier) -> Tuple[Optional[str], Optional[str]]:
        ...
        Returns (column_name, table_name) where table_name is None if unqualified.
        """
        if hasattr(identifier, 'get_real_name'):
            name = identifier.get_real_name()
            if name:
                # Clean up the name - remove DESC, ASC, etc.
